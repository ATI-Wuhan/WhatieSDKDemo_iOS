...
- (void)viewDidLoad {
    ...
    self.title = NSLocalizedStringFromTable(@"Feedback", @"Profile", nil);
    
    self.feedbackArray = [NSMutableArray array];
    
    UIBarButtonItem *addFeedbackItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedStringFromTable(@"Add", @"Profile", nil) style:UIBarButtonItemStylePlain target:self action:@selector(addFeedback)];
